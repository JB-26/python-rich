...
def delete_game(access_token):
    ...
    console.print("[yellow]Enter the [bold]ID[/bold] of the game in the database you want to delete[/yellow]")
    console.print("[bold red]WARNING! This will remove the game and cannot be recovered![/bold red]")
    delete_id = int(input())
    ...
    if r.status_code != 200:
        ...
        table.add_row(f"{result['ID: ']}", f"{result['Name: ']}", f"{result['Genre: ']}", f"{result['Platform: ']}", f"{result['Publisher: ']}", f"{result['Year']}")
