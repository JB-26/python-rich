...
def add_game(access_token):
    ...
    try:
        console.print(
            "[italic yellow]Please enter the following details[/italic yellow]")

        console.print(
            "[yellow]Enter the [bold]name[/bold] of the game[/yellow]")
        add_name = input()

        console.print(
            "[yellow]Enter the [bold]platform[/bold] of the game[/yellow]")
        add_platform = input()

        console.print(
            "[yellow]Enter the [bold]publisher[/bold] of the game[/yellow]")
        add_publisher = input()

        console.print(
            "[yellow]Enter the [bold]genre[/bold] of the game[/yellow]")
        add_genre = input()

        console.print(
            "[yellow]Enter the [bold]year[/bold] when the game was released[/yellow]")
        add_year = int(input())
        ...
        with console.status(f"[purple]Now adding {add_name}... 🎮[/purple]") as status:
            r = requests.post(f'{apiUrl}/addGame',
        ...
        if r.status_code != 201:
            ...
            table.add_row(f"{add_name}", f"{add_genre}",
                          f"{add_platform}", f"{add_publisher}", f"{add_year}")
            console.print(table)
